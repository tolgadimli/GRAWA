...
class Worker(WorkerDistOptim, WorkerStocOptim):
    def __init__(self, args, cur_worker, shared_tensor, shared_lock, shared_queue_r, shared_queue_a):
        ...
        #print(self.shared_queue_a[self.my_gpu])
        self.train_time = 0
        self.test_time = 0
        ...
        while not train_stop:
            ...
            while batch_idx < train_iterator_len:
                ...
                if message == 0: # local training
                    ...
                    # LR Drop for ResNets
                    if self.args.model == 'vgg16' and self.raw_time >= 1200 and flag1:
                        for param_group in self.local_optimizer.param_groups:
                            self.cur_lr = self.cur_lr* 0.1
                            self.local_cur_lr = self.cur_lr
                            param_group['lr'] = self.cur_lr
                            flag1 = False

                    elif self.args.model == 'resnet20' and self.raw_time >= 500 and flag1:
                        for param_group in self.local_optimizer.param_groups:
                            self.cur_lr = self.cur_lr* 0.1
                            self.local_cur_lr = self.cur_lr
                            param_group['lr'] = self.cur_lr
                    ...
                    elif self.args.model == 'resnet50' and self.epoch % 35 == 0:
                        for param_group in self.local_optimizer.param_groups:
                            param_group['lr'] = self.cur_lr* (0.1 ** (self.epoch // 35))
                        self.local_cur_lr = self.cur_lr* (0.1 ** (self.epoch // 35))

                    elif self.args.model == 'pyramidnet' and self.epoch % 150 == 0:
                        for param_group in self.local_optimizer.param_groups:
                            param_group['lr'] = self.cur_lr * (0.1 ** (self.epoch // 150))
                        self.local_cur_lr = self.cur_lr* (0.1 ** (self.epoch // 150))
                    elif self.args.model == 'pyramidnet' and self.epoch % 225 == 0:
                        for param_group in self.local_optimizer.param_groups:
                            param_group['lr'] = self.cur_lr * (0.01 ** (self.epoch // 225))
                        self.local_cur_lr = self.cur_lr* (0.01 ** (self.epoch // 225))

                    elif self.args.model == 'densenet' and self.epoch == 200:
                        for param_group in self.local_optimizer.param_groups:
                            param_group['lr'] = self.cur_lr * 0.1
                        self.local_cur_lr = self.cur_lr * 0.1

                    elif self.args.model == 'wideresnet' and self.epoch == 160:
                        for param_group in self.local_optimizer.param_groups:
                            param_group['lr'] = self.cur_lr * 0.2
                        self.local_cur_lr = self.cur_lr * 0.2

                    elif self.args.model == 'resnet56' and self.epoch == 150:
                        for param_group in self.local_optimizer.param_groups:
                            param_group['lr'] = self.cur_lr * 0.1
                        self.local_cur_lr = self.cur_lr * 0.1
                    ...
                    # if you want per epoch testing use this instead: test_gap = len(self.train_loader.dataset) // (self.args.batch_size * self.args.num_groups *  self.args.num_gpus * self.args.g_comm) 
                    test_gap = len(self.train_loader.dataset) // (self.args.batch_size * self.args.num_groups * self.args.g_comm ) 
                    ...
                    torch.save(state, os.path.join(expr_dir,'checkpoint-w%d-end.pt'%self.my_rank))
                    train_stop = True
                    break
