...
from optimizers import GRAWAOptimWrapper
...
class WorkerStocOptim(WorkerBase):
    def __init__(self, args, cur_worker, shared_tensor, shared_lock, shared_queue_r, shared_queue_a):
        ...
        if "GRAWAWrapper" in args.dist_optimizer:
            print('Using the GRAWA Wrapper')
            base_optim = optim.SGD
            grawa_layer_size = len([n for n,p in self.model.named_parameters() if p.requires_grad])
            self.local_optimizer = GRAWAOptimWrapper(self.model.parameters(), grawa_beta=0.85, grawa_layer_size = grawa_layer_size,
                                        base_optimizer=base_optim, lr = self.cur_lr, weight_decay = args.wd, momentum = args.mom, nesterov = (args.mom>0))
    ...
    def local_batch_train(self, data, target):
        ...
        if self.args.grawa_prox:
            unravel_model_params(self.model, self.dist_params_tensor_msr, is_grad=False, operation='mix', model_weight=1-p2)
            #print('INSIDE')
    ...
    def local_center_test(self):
        ...
        with torch.no_grad():
            ...
            if 'net' in self.args.model or 'vgg' in self.args.model:
                print('Handling Batchnorm...')
