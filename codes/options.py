...
class Options():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--avg_size', default=4, type=int)
        
        parser.add_argument('--landscape',  action='store_true')
        parser.add_argument('--suffix', default='', type=str) # additional parameters
        parser.add_argument('--grawa_prox', default=False, action='store_true')
        parser.add_argument('--grawa_momentum', default=False, action='store_true')
    ...
    def parse(self):
        ...
        assert args.dist_optimizer in ['EASGD', 'LSGD', 'MGRAWA', 'LGRAWA', 'MGRAWAWrapper', 'LGRAWAWrapper'], 'No such distributed optimizer supported'
        if not args.dist_optimizer == 'LSGD':
            args.no_prox = True

        # args.grawa_prox = False
        # if 'GRAWA' in args.dist_optimizer:
        #     args.grawa_prox = True

        print(args.grawa_prox)
