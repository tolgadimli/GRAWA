...
import math
import argparse
from models import ResNet20, VGG16, PyramidNet
from optimizers import LARS, SAM
...
def setup(rank, world_size = 12, IP = '128.238.9.108'):
    os.environ['MASTER_ADDR'] = IP
    os.environ['MASTER_PORT'] = '2432'
...
def main_worker(gpu, args):
    ...
    setup(args.num_gpus* args.cur_group + gpu, args.world_size, args.dist_ip)
    if args.dataset == 'CIFAR10' or args.dataset == 'CIFAR100':
        ...
        train_dataset = datasets.__dict__[args.dataset](args.datadir, train=True, download=True, transform=transformTR)
        test_dataset = datasets.__dict__[args.dataset](args.datadir, train=False, transform=transformTE)

    elif args.dataset == 'imagenet':
        normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                          std=[0.229, 0.224, 0.225])
        kwargs = {'num_workers': 4, 'pin_memory': True} # num_workers is the same as LSGD paper
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, args.batch_size, sampler=train_sampler, drop_last=True, **kwargs)
    ...
    if args.model == 'vgg16':
        ...
        model = PyramidNet(110, 270, args.num_classes).to('cuda:0')
        print('Using PyramidNet')
    elif args.model == 'resnet50':
        model = torchvision.models.resnet50(pretrained=False).to('cuda:0')
    else:
    ...
    if args.optimizer == 'SGD':
        ...
        optimizer = SAM(model.parameters(), torch.optim.SGD , lr=args.lr, momentum=args.mom, weight_decay=args.wd)
    else:
    ...
    expected_epochs = torch.cuda.IntTensor([expected_epochs])
    dist.broadcast_multigpu([expected_epochs], src = 0)    
    expected_epochs = args.epochs if args.use_epochs else expected_epochs.item()
    #expected_epochs = 90
    args.remaining_epochs =  expected_epochs - 1
    args.remaining_steps  = (expected_epochs - 1)* len(train_loader)
    if gpu % args.num_gpus == 0:
    ...
    for epoch in range(1, expected_epochs):
        ...
        if args.optimizer == 'SGD':
            if args.model == 'vgg16' and total_time >= 500:
                ...
                for param_group in optimizer.param_groups:
                        param_group['lr'] = args.lr* 0.1
            elif args.model == 'resnet50' and epoch % 35 == 0:
                for param_group in optimizer.param_groups:
                        param_group['lr'] = args.lr* (0.1** (epoch//35))

            elif args.model == 'pyramidnet' and args.optimizer != 'SAM' and epoch % 150 == 0:
                for param_group in optimizer.param_groups:
                        param_group['lr'] = args.lr* (0.1** (epoch//150))
            elif args.model == 'pyramidnet' and args.optimizer != 'SAM' and epoch % 225 == 0:
                for param_group in optimizer.param_groups:
                        param_group['lr'] = args.lr* (0.01** (epoch//225))

            elif args.model == 'pyramidnet' and args.optimizer == 'SAM' and epoch % 100 == 0:
                for param_group in optimizer.param_groups:
                        param_group['lr'] = args.lr* (0.1** (epoch//100))
            elif args.model == 'pyramidnet' and args.optimizer == 'SAM' and epoch % 150 == 0:
                for param_group in optimizer.param_groups:
                        param_group['lr'] = args.lr* (0.01** (epoch//150))

        elif args.optimizer == 'LARS':
            pass
        total_time += time.time() - strat_time
        
        log_internval = 32 if args.dataset == 'imagenet' else 128
        if epoch % (log_internval // args.batch_size) == 0:
            test_loss, test_error = test(args, model, device, test_loader)
            message_file = open(file_name, "a")
    ...
    if args.save_model and gpu % args.num_gpus == 0:
        save_name = os.path.join(exp_dir, 'modelparams.pt')
